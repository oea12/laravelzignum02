...
Input;
  /*
  |--------------------------------------------------------------------------
  | Backend controller for \Thor\Models\Fruit 
  |--------------------------------------------------------------------------
  |
  ...
  |
  */
  class FruitsController extends \Thor\Backend\Controller {

      /**
       * Model repository
       *
       * @var \Thor\Models\Fruit     */
      protected $record;
      ...
      */
      ...
      public function __construct(\Thor\Models\Fruit $record) {
        $this->record = $record;
        $this->destination_path= base_path().'/public/img/fruit';
      }
      ...
      public function index() {
        ...
        $module = \Thor\Models\Fruit::relatedModule();
        return View::make('thor::backend.fruits.index', compact('records', 'module'));
      }
      /**
      ...
      */
       ...
       *
       * @param  \Thor\Models\Fruit  $record 
       * @return Response
       */
      public function edit(\Thor\Models\Fruit $record) {
        if (is_null($record)) {
          return Redirect::route('backend.fruits.index');
        }
        $module = \Thor\Models\Fruit::relatedModule();

        return View::make('thor::backend.fruits.edit', compact('record', 'module'));
      }
       ...
       * @return Response
       */
      public function do_edit(\Thor\Models\Fruit $record) {
        /*----------*/
        ...
        $transl_record = new \Thor\Models\FruitText();
        $transl_errors = array();
        if ($record->validate($input)) {
          ...
          if (Input::hasFile('icon')){
            ...
            $input['icon']= \Thor\Models\Fruit::find($record->id)->icon;
          } 
          /*----------*/
          $record->update($input);
          if ($transl_record->validate($transl_input)) {
            ...
            if (Input::get('translation.id')) {
              $transl_record = \Thor\Models\FruitText::find(Input::get('translation.id'));
              $transl_record->update($transl_input);
            } else {
              $transl_record = $transl_record->create(array_merge(array('language_id' => \Lang::id(), 'fruit_id' => $record->id), $transl_input));
            }
            $records = $this->record->all();
            $module = \Thor\Models\Fruit::relatedModule();
            return View::make('thor::backend.fruits.index', compact('records', 'module'));
        ...
        return Redirect::route('backend.fruits.edit', $record->id)
        ->withInput()
