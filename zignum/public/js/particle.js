...
$(window).load(function(){
	...
	if(mWidth > 800){
		$('.sparkling').on('click',function(pos){
			...
			if (now - last_moved > delay) {
				...
				for(var i=1;i<=particle_number;++i){
					...
					$(this).append('<img class="particle" src="' + imgsrc + (1+Math.round(rand1 * (spark_types-1))) + '.png"' + "/>");
					...
					xpos *= Math.abs(Math.floor(last_x - 15))/accel_div;
					ypos *= Math.abs(Math.floor(last_y - 15))/accel_div;
							
					//set particle size and position
					var vari= Math.floor(particle_variation + rand1 * particle_variation);
					var startt = 15 - vari*(1 - Math.round(rand2));
					var startl = 15 - vari*(1 - Math.round(rand1));
...
var setStyle = {
	...
	snow:	function (){
		particle_number    = 5; //how many particles to be launched at every mouse move
		...
		dir           = -1; // 1: particles follow the mouse cursor || -1: paricles go the opposite way
		particle_life = 500; //how long the particle lasts before gone (speed) [miliseconds]
