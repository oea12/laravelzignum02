...
<style type="text/css">
.movil ul li:nth-child(10) p {
	background: url(/zignum/public/img/basesubmenu_on.jpg);	
	background-size: cover;
	background-position: top;
	background-repeat: no-repeat;
}
#cocteles, #cocteles a img:nth-child(2), #silver, #silver a img:nth-child(2) {
...
<section class="contain">
		...
		<?php
		  $coc_lan = "";
			if($_COOKIE['lengua']==='en'){
				$coc_lan = '<p class="title">ZIGNUM SILVER <span>COCKTAILS</span></p>';
			}else{
				$coc_lan='<p class="title"><span>COCTELES</span> ZIGNUM SILVER</p>';
			}
		?>
		{{$coc_lan}}
		@if (count($cocktail) >= 1)
		<div id="carrusel">
			<ul id="carrusel_img">
				@foreach ($cocktail as $cocktails)
		    		<li>
		    			<img src="/zignum/public/img/coctel/{{ $cocktails->cocktailimage }}" alt="Cocteles Zignum Silver" title="Zignum Silver Mezcal">
		    		</li>
				@endforeach
			</ul>
		    			<!-- <img onclick='video("{{ $cocktails->video_mp4 }}", "{{ $cocktails->video_ogg }}", "{{ $cocktails->web }}")' src="img/coctel/{{ $cocktails->cocktailimage }}" alt="Cocteles Zignum Silver" title="Zignum Silver Mezcal"> -->
			<ul id="carrusel_desc">
				@foreach ($cocktail as $cocktails)
					<?php
						if($_COOKIE['lengua']==='en'){
	...
	@if (count($cocktail) >= 1)
	<div id="carrusel_selector" class="selector_silver">
		<ul>
				@foreach ($cocktail as $cocktails)
		   		<?php
		   			$coc = "";
		   		...
		   		<img src="/zignum/public/img/coctel/icon/{{ $cocktails->cocktailicon }}" alt="{{isset($alt)?$alt:'Cocteles Zignum Silver'}}" title="{{isset($title)?$title:'Zignum Silver Mezcal'}}">
		   		</li>
				@endforeach
		</ul>
	</div>
