...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">@if($module)<i class="module-icon fa {{$module->icon}}"></i>@endif  Edit Fruit</h1>

        <p>{{ link_to_route('backend.fruits.index', 'Return to all fruits') }}</p>
    ...
    , 'route' => array('backend.fruits.do_edit', $record->id), 'role'=>'form','file' => true, 'enctype' => 'multipart/form-data')) }}
<?php
...
?>{{ Form::hidden('translation[id]', $transl->id) }}
        ...
        <div class="form-group">
            ...
            {{ link_to_route('backend.fruits.index', 'Cancel', array($record->id), array('class' => 'btn btn-default')) }}
        </div>
